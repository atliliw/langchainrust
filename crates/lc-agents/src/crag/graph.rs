...
impl<'a, M: BaseChatModel, R: RetrieverTrait> CRAGGraph<'a, M, R> {
    ...
    pub(crate) async fn correct(&self, state: &mut CRAGState) -> Result<(), CRAGError> {
        ...
        if !all_docs.is_empty() {
            ...
            state.current_query = alternatives
                .into_iter()
                .next()
                .unwrap_or(state.query.clone());
...
mod tests {
    ...
    fn test_truncate_docs_by_tokens_no_limit() {
        let docs = vec![Document::new("Short doc"), Document::new("Another doc")];
    ...
    fn test_hallucination_check_word_boundary_not_grounded() {
        ...
        assert!(
            not_grounded,
            "'not grounded' should be detected as not grounded"
        );
    }
    ...
    fn test_hallucination_check_word_boundary_grounded() {
        ...
        assert!(
            !not_grounded,
            "'grounded' (without 'not') should NOT be detected as not grounded"
        );
        assert!(
            words.contains(&"grounded"),
            "should contain 'grounded' keyword"
        );
    }
    ...
    fn test_hallucination_check_ambiguous_defaults_not_grounded() {
        ...
        let not_grounded = words.windows(2).any(|w| w == ["not", "grounded"])
            ...
            || words.contains(&"unsupported");
        let is_grounded =
            words.contains(&"grounded") || words.contains(&"yes") || words.contains(&"supported");

...
pub(super) fn build_plan_config(
...
impl AgentExecutor {
    ...
    pub async fn resume(&self, decision: ApprovalDecision) -> Result<Option<String>, AgentError> {
        ...
        // T10: root run is a GenAI agent invocation, not a generic chain.
    ...
    async fn invoke_inner(
        ...
        // T10: classify the root run as the 2026 `invoke_agent` GenAI operation
        // before any handler sees on_chain_start, so OtelHandler names the span
        // `invoke_agent` (unstamped chain runs still produce "chain").
        root_run = root_run.with_metadata(GEN_AI_OPERATION_NAME, json!("invoke_agent"));

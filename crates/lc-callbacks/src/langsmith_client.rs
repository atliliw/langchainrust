// lc-callbacks/src/langsmith_client.rs
...
impl LangSmithConfig {
    ...
    pub fn from_env() -> Result<Self, String> {
        ...
        let project_name = env::var("LANGSMITH_PROJECT").unwrap_or_else(|_| "default".to_string());
...
impl LangSmithClient {
    ...
    pub async fn batch_ingest(&self, runs: &[RunTree]) -> Result<(), LangSmithError> {
        ...
        let runs_json =
            serde_json::to_string(&runs_create).map_err(|e| LangSmithError::Api(e.to_string()))?;

        let form = reqwest::multipart::Form::new().text("runs", runs_json);
    ...
    pub async fn batch_ingest_parallel(&self, runs: &[RunTree]) -> Result<(), LangSmithError> {
        ...
        let futures: Vec<_> = runs.iter().map(|run| self.create_run(run)).collect();

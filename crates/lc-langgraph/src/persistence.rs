// crates/lc-langgraph/src/persistence.rs
...
impl FilePersistence {
    ...
    fn graph_path(&self, id: &str) -> Result<PathBuf, PersistenceError> {
        // Sanitize id to prevent path traversal: reject ".." and absolute paths
        if id.contains("..") || id.contains('/') || id.contains('\\') {
            return Err(PersistenceError::InvalidDefinition(format!(
                "Invalid graph id '{}': path traversal detected",
                id
            )));
        }
        if std::path::Path::new(id).is_absolute() {
            return Err(PersistenceError::InvalidDefinition(format!(
                "Invalid graph id '{}': absolute path not allowed",
                id
            )));
        }
            ...
            Ok(Self {
                uri: std::env::var("MONGO_URI").map_err(|_| {
                    PersistenceError::ConnectionError(
                        "MONGO_URI environment variable not set".to_string(),
                    )
                })?,
...
#[cfg(feature = "mongodb-persistence")]
pub use mongo_impl::{MongoConfig, MongoPersistence};
...
mod tests {
    ...
    async fn test_memory_persistence() {
        ...
        let def = GraphDefinition::new("entry".to_string()).with_id("test-001".to_string());

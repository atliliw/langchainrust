...
use langchainrust::{
    ...
    BaseTool, CallbackHandler, CallbackManager, RunTree,
};
...
impl CallbackHandler for ToolTrackingHandler {
    async fn on_run_start(&self, run: &RunTree) {
        self.calls
            .lock()
            .unwrap()
            .push(format!("run_start:{}", run.name));
    }
    ...
    async fn on_run_error(&self, run: &RunTree, error: &str) {
        self.calls
            .lock()
            .unwrap()
            .push(format!("run_error:{}:{}", run.name, error));
    }
    ...
    async fn on_chain_error(&self, run: &RunTree, error: &str) {
        self.calls
            .lock()
            .unwrap()
            .push(format!("chain_error:{}:{}", run.name, error));
    }
    ...
    async fn on_tool_start(&self, run: &RunTree, tool_name: &str, input: &str) {
        ...
        *count += 1;
        ...
        self.tool_durations.lock().unwrap().push((
            format!("{}_start", tool_name),
    ...
    async fn on_tool_end(&self, run: &RunTree, output: &str) {
        ...
        self.calls
            .lock()
            .unwrap()
            .push(format!("tool_end:{}:{}", run.name, output));
        ...
        *count += 1;
        ...
        self.tool_outputs
            .lock()
            .unwrap()
            .push((tool_name.clone(), output.to_string()));

        // 记录结束时间
        let end_time = std::time::Instant::now();
        self.tool_durations.lock().unwrap().push((
            format!("{}_end", tool_name),
            end_time.elapsed().as_millis() as i64,
        ));
    }
    ...
    async fn on_tool_error(&self, run: &RunTree, error: &str) {
        ...
        self.calls
            .lock()
            .unwrap()
            .push(format!("tool_error:{}:{}", run.name, error));
    ...
    async fn on_llm_start(&self, run: &RunTree, _messages: &[langchainrust::schema::Message]) {
        self.calls
            .lock()
            .unwrap()
            .push(format!("llm_start:{}", run.name));
    }
    ...
    async fn plan(
        ...
        if intermediate_steps.is_empty() {
            return Ok(AgentOutput::Action(AgentAction {
                ...
                tool_input: langchainrust::agents::ToolInput::String {
...
impl BaseAgent for MultiToolAgent {
    async fn plan(
        ...
        // 所有工具调用完成后，返回结果
        let observations = intermediate_steps
            .iter()
...
async fn test_single_tool_callback_chain() {
    ...
    // 创建 Executor
    let executor = AgentExecutor::new(agent, tools).with_callbacks(callbacks);
    ...
    // 验证回调被触发
    assert_eq!(
        handler.get_tool_start_count(),
        1,
        "on_tool_start 应被调用 1 次"
    );
...
async fn test_multiple_tools_callback_chain() {
    ...
    let agent = Arc::new(MultiToolAgent::new(vec![
        ...
        (
            "math".to_string(),
            "{\"operation\": \"sqrt\", \"value\": 100}".to_string(),
        ),
    ]));
    ...
    assert!(calls
        .iter()
        .any(|c| c.contains("tool_start") && c.contains("calculator")));
    assert!(calls
        .iter()
        .any(|c| c.contains("tool_start") && c.contains("math")));
}
...
async fn test_tool_error_callback() {
    ...
    // 应该有 chain 生命周期
    assert!(
        calls.iter().any(|c| c.starts_with("chain_start")),
        "应有 chain_start"
    );
    assert!(
        calls.iter().any(|c| c.starts_with("chain_error")),
        "应有 chain_error"
    );
...
async fn test_tool_invalid_input_callback() {
    ...
    // 如果工具失败，验证 error 被调用
    let total_callbacks = handler.get_tool_start_count()
        + handler.get_tool_end_count()
        + handler.get_tool_error_count();
...
async fn test_full_execution_trace() {
    ...
    assert!(
        calls.iter().any(|c| c.starts_with("chain_end")),
        "应有 chain_end"
    );

    // 应包含 tool 生命周期
    assert!(
        calls.iter().any(|c| c.starts_with("tool_start")),
        "应有 tool_start"
    );
    assert!(
        calls.iter().any(|c| c.starts_with("tool_end")),
        "应有 tool_end"
    );

    // 验证顺序：chain_start → tool_start → tool_end → chain_end
    let chain_start_idx = calls
        .iter()
        .position(|c| c.starts_with("chain_start"))
        .unwrap();
    let tool_start_idx = calls
        .iter()
        .position(|c| c.starts_with("tool_start"))
        .unwrap();
    let tool_end_idx = calls
        .iter()
        .position(|c| c.starts_with("tool_end"))
        .unwrap();
    let chain_end_idx = calls
        .iter()
        .position(|c| c.starts_with("chain_end"))
        .unwrap();

    assert!(
        chain_start_idx < tool_start_idx,
        "chain_start 应在 tool_start 之前"
    );
    assert!(
        tool_start_idx < tool_end_idx,
        "tool_start 应在 tool_end 之前"
    );
...
async fn test_multiple_handlers_receive_tool_events() {
    ...
    // 验证两个处理器都收到了回调
    assert_eq!(
        handler1.get_tool_start_count(),
        1,
        "handler1 应收到 tool_start"
    );
    assert_eq!(
        handler2.get_tool_start_count(),
        1,
        "handler2 应收到 tool_start"
    );

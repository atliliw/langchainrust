...
async fn test_conditional_routing_short_path() {
    ...
    let short_result = compiled
        .invoke(AgentState::new("hi".to_string()))
        .await
        .unwrap();
...
async fn test_conditional_routing_long_path() {
    ...
    // 长输入应路由到 long 节点
    let long_result = compiled
        .invoke(AgentState::new("this is a very long input".to_string()))
        .await
        .unwrap();
...
async fn test_conditional_routing_with_default() {
    ...
    let default_result = compiled
        .invoke(AgentState::new("xyz".to_string()))
        .await
        .unwrap();
    assert_eq!(
        default_result.final_state.output,
...
async fn test_multiple_conditional_branches() {
    ...
    // 路由: 输入内容直接决定路径
    let router = FunctionRouter::new(|state: &AgentState| state.input.clone());
    graph.set_conditional_router("branch_router", router);

    let compiled = graph.compile().unwrap();

    // 测试三条路径都能正确路由
    let alpha_result = compiled
        .invoke(AgentState::new("alpha".to_string()))
        .await
        .unwrap();
    assert_eq!(
        alpha_result.final_state.output,
        Some("alpha_result".to_string())
    );

    let beta_result = compiled
        .invoke(AgentState::new("beta".to_string()))
        .await
        .unwrap();
    assert_eq!(
        beta_result.final_state.output,
        Some("beta_result".to_string())
    );

    let gamma_result = compiled
        .invoke(AgentState::new("gamma".to_string()))
        .await
        .unwrap();
    assert_eq!(
        gamma_result.final_state.output,
        Some("gamma_result".to_string())
    );
}
...
fn test_conditional_edge_validation_requires_router() {
    ...
    let targets = HashMap::from([("route".to_string(), "target".to_string())]);
    // 添加条件边但没有设置路由器

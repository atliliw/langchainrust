use langchainrust::{
    AgentState, FunctionRouter, MessageEntry, StateGraph, StateUpdate, END, START,
};
...
async fn example_smart_qa_pipeline() {
    ...
    graph.add_node_fn("analyze_complexity", |state: &AgentState| {
        ...
        let mut new_state = state.clone();
        new_state.add_message(MessageEntry::ai(format!(
            "分析完成 - 问题类型: {}",
            if is_complex { "复杂" } else { "简单" }
        )));
        Ok(StateUpdate::full(new_state))
    });
    ...
    // 测试简单问题
    let simple_result = compiled
        .invoke(AgentState::new("什么是 Rust?".to_string()))
        .await
        .unwrap();
    ...
    // 测试复杂问题
    let complex_result = compiled
        .invoke(AgentState::new(
            "请详细解释所有权系统和生命周期概念?".to_string(),
        ))
        .await
        .unwrap();
...
async fn example_multi_branch_routing() {
    ...
    let router = FunctionRouter::new(|state: &AgentState| {
        {
            ...
            if input.contains("计算") || input.contains("数学") {
                "math"
            } else if input.contains("翻译") || input.contains("写作") {
                "text"
            } else if input.contains("代码") || input.contains("编程") {
                "code"
            } else {
                "unknown"
            }
    ...
    // 测试四种类型
    let math_result = compiled
        .invoke(AgentState::new("帮我计算 1+1".to_string()))
        .await
        .unwrap();
    assert!(math_result
        .final_state
    ...
    let text_result = compiled
        .invoke(AgentState::new("请翻译这段文字".to_string()))
        .await
        .unwrap();
    assert!(text_result
        .final_state
    ...
    let code_result = compiled
        .invoke(AgentState::new("写一段排序代码".to_string()))
        .await
        .unwrap();
    assert!(code_result
        .final_state
    ...
    let default_result = compiled
        .invoke(AgentState::new("今天天气怎么样".to_string()))
        .await
        .unwrap();
    ...
    println!(
        "数学 -> {}",
        math_result.final_state.output.as_ref().unwrap()
    );
    println!(
        "文本 -> {}",
        text_result.final_state.output.as_ref().unwrap()
    );
    println!(
        "代码 -> {}",
        code_result.final_state.output.as_ref().unwrap()
    );
    println!(
        "未知 -> {}",
        default_result.final_state.output.as_ref().unwrap()
    );
}
...
async fn example_loop_with_limit() {
    ...
    let result = compiled
        .invoke(AgentState::new("0".to_string()))
        .await
        .unwrap();
    assert!(result
        .final_state

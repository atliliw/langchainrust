...
use langchainrust::{AgentState, GraphBuilder, GraphExecution, StateUpdate, END, START};
...
async fn test_interrupt_before() {
    ...
    assert!(matches!(
        err,
        langchainrust::GraphError::ExecutionInterrupted(_)
    ));
...
async fn test_multiple_interrupts() {
    ...
    execution2
        .state
        .add_message(langchainrust::MessageEntry::ai("check1_pass".to_string())); // 模拟 check1 的结果

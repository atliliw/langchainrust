...
async fn test_compression_trigger() {
    ...
    for round in 1..=5 {
        ...
        println!(
            "保存前: chat_memory={} 条, buffer=\"{}\"",
            before_count, before_buffer
        );
        ...
        match result {
            Ok(_) => {
                ...
                println!(
                    "保存后: chat_memory={} 条, buffer=\"{}\"",
                    after_count, buffer_display
                );
                ...
                assert!(
                    !after_buffer.is_empty() || after_count > 0,
                    "应有摘要或消息"
                );
            }
            Err(e) => {

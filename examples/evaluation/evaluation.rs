...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = evaluator.eval("What language?", "Rust is a systems programming language", "").await?;
    ...
    let result = evaluator.eval("Explain", "This is a medium length response", "").await?;
    println!("LengthCheck: score = {}", result.value);

    // 5. StringDistance 评测器
    let evaluator = StringDistance;
    let result = evaluator.eval("Greet", "hello world", "hello world").await?;
    println!("StringDistance (相同): score = {}", result.value);

    // 6. Bleu 评测器
    let evaluator = Bleu::new();
    let result = evaluator.eval("Translate", "the cat is on the mat", "the cat sat on the mat").await?;

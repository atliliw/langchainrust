...
impl BaseChain for SequentialChain {
    ...
    async fn invoke(&self, inputs: HashMap<String, Value>) -> Result<ChainResult, ChainError> {
        ...
        for (step_index, step) in self.chains.iter().enumerate() {
            ...
            // 更新状态
            // H68: Return error when expected output key is missing instead of silently skipping
...
mod tests {
    use super::*;
    use crate::{LLMChain, OpenAIChat, OpenAIConfig};
    ...
    async fn test_sequential_chain_mock() {
        ...
        // 执行
        let inputs = HashMap::from([("text".to_string(), Value::String("hello".to_string()))]);
    ...
    async fn test_sequential_chain_real() {
        ...
        // 执行
        let inputs = HashMap::from([("topic".to_string(), Value::String("编程".to_string()))]);
    ...
    async fn test_sequential_chain_three_steps() {
        ...
        let inputs = HashMap::from([(
            "text".to_string(),
            Value::String("Rust是一门系统编程语言，注重安全和性能".to_string()),
        )]);

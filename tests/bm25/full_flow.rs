...
use langchainrust::{
    ChunkedDocumentStoreTrait, Document, Embeddings, HybridIndexConfig, MockEmbeddings,
    UnifiedHybridIndex,
};
use std::sync::Arc;
...
async fn test_full_flow_with_print() {
    ...
    let docs = vec![
        Document::new("Rust是一门系统编程语言，注重安全和性能。所有权系统是核心特性。")
            .with_id("doc_001"),
        Document::new("Python是一门高级编程语言，适合数据科学和机器学习。").with_id("doc_002"),
        Document::new("内存管理是系统编程的重要部分，包括栈分配和堆分配。").with_id("doc_003"),
        Document::new("Rust的所有权规则确保内存安全，无需垃圾回收。").with_id("doc_004"),
    ];
...
async fn test_large_document_full_flow() {
    ...
    println!(
        "  内容预览: {}...",
        full_doc.content.chars().take(50).collect::<String>()
    );
    println!();
    ...
    let chunks = store
        .get_chunks_for_parent("large_doc")
        .await
        .expect("获取失败");

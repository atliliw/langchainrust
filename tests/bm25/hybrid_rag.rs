...
use langchainrust::retrieval::bm25::{ChunkedBM25Retriever, AutoMergingConfig};
use langchainrust::retrieval::{HybridRetriever, reciprocal_rank_fusion};
use langchainrust::{BaseChatModel, Document, InMemoryVectorStore, Message};
use langchainrust::retrieval::ChunkedDocumentStore;
use langchainrust::{SimilarityRetriever, RetrieverTrait};
use std::sync::Arc;
...
fn test_rrf_fusion() {
    ...
    for (i, doc) in bm25_docs.iter().enumerate() {
        println!("  [{}] id={}, 内容={}", i, doc.id.clone().unwrap_or_default(), doc.content);
    }
    ...
    for (i, r) in results.iter().enumerate() {
        println!("  [{}] id={}, rrf_score={:.4}", 
            i, r.document.id.clone().unwrap_or_default(), r.score);
    }
...
fn test_hybrid_retriever_usage() {
    ...
    for r in &results {
        println!("  id={}, score={:.4}", 
            r.document.id.clone().unwrap_or_default(), r.score);
    }
}
...
async fn test_hybrid_rag_with_llm() {
    ...
    vector_retriever.add_documents(knowledge_docs.clone()).await.unwrap();
    ...
    for (i, result) in bm25_results.iter().enumerate() {
        println!("  [{}] 分数={}, 是否合并={}, 内容={}", 
            i, result.score, result.is_merged(), result.content());
    }
    ...
    // [9] 构建上下文
    let contexts: Vec<String> = fused_results.iter()
        .take(3)
...
async fn test_hybrid_rag_query_types() {
    ...
    let knowledge_docs = vec![
        Document::new("机器学习监督学习无监督学习强化学习是AI的核心技术。")
            .with_id("ml_doc"),
        Document::new("深度学习使用神经网络自动提取特征，应用于图像识别。")
            .with_id("dl_doc"),
        Document::new("自然语言处理让计算机理解人类语言，应用包括翻译问答。")
            .with_id("nlp_doc"),
    ];
    ...
    for (query, query_type) in queries {
        ...
        for r in &fused_results {
            println!("  id={}, score={:.4}", r.document.id.clone().unwrap_or_default(), r.score);
        }
...
async fn test_hybrid_rag_with_persistence() {
    ...
    let knowledge_docs = vec![
        Document::new("人工智能AI是计算机科学分支。")
            .with_id("ai_doc"),
        Document::new("机器学习ML是AI核心技术。")
            .with_id("ml_doc"),
    ];

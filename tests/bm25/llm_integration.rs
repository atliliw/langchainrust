...
async fn test_bm25_rag_with_openai_llm() {
    ...
    retriever.add_documents_async(vec![
        Document::new(
    ...
    for (i, result) in results.iter().enumerate() {
        println!("  [{}] 分数={}, 是否合并={}", i, result.score, result.is_merged());
...
async fn test_bm25_rag_with_large_context() {
    ...
    println!("BM25 检索结果:");
    for (i, result) in results.iter().enumerate() {
        println!("  [{}] 是否合并={}, 分数={}", i, result.is_merged(), result.score);
...
async fn test_bm25_rag_chinese_query() {
    ...
    for (i, result) in results.iter().enumerate() {
        println!("  [{}] 分数={}, 是否合并={}", i, result.score, result.is_merged());
        println!("      内容包含关键词: {}", 
            if result.content().contains("强人工智能") { "强人工智能" } else { "" });
    }

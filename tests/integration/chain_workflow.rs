...
use langchainrust::{BaseChain, ChatMessageHistory, LLMChain, SequentialChain};
...
async fn test_chain_with_memory() {
    ...
    inputs.insert(
        "question".to_string(),
        Value::String("My favorite color is blue.".to_string()),
    );
...
async fn test_sequential_chain_data_flow() {
    ...
    let chain1 = LLMChain::new(
        config.openai_chat(),
        "Extract 3 key topics from: {question}.",
    )
    .with_input_key("question")
    ...
    inputs.insert(
        "question".to_string(),
        Value::String("AI and Machine Learning".to_string()),
    );

...
use langchainrust::{
    Document, Embeddings, InMemoryVectorStore, RecursiveCharacterSplitter, RetrieverTrait,
    SimilarityRetriever, TextSplitter, VectorStore,
};
use std::sync::Arc;

#[cfg(feature = "qdrant-integration")]
use langchainrust::vector_stores::{QdrantConfig, QdrantDistance, QdrantVectorStore};
...
async fn test_inmemory_embeddings_real() {
    ...
    assert!(
        results[0].document.metadata.get("lang") == Some(&"rust".to_string()),
        "第一个结果应该是 Rust 文档"
    );
}
...
async fn test_rag_inmemory_full_pipeline() {
    ...
    let knowledge_docs = vec![
        ...
        Document::new("Rust 的所有权系统确保内存安全，无需垃圾回收。")
            .with_metadata("source", "wiki"),
        Document::new("Cargo 是 Rust 的包管理器和构建工具。").with_metadata("source", "wiki"),
        Document::new("Rust 支持异步编程，使用 async/await 语法。").with_metadata("source", "wiki"),
        Document::new("Rust 编译器非常严格，能捕获许多潜在错误。").with_metadata("source", "wiki"),
    ];
    ...
    // 验证答案包含关键信息
    assert!(
        response.content.contains("2015"),
        "答案应该包含发布年份 2015"
    );
}
...
async fn test_rag_with_document_splitting() {
    ...
    let chunks: Vec<Document> = splitter
        .split_text(&long_doc.content)
        .into_iter()
    ...
    assert!(
        response.content.contains("Cargo") || response.content.contains("包管理"),
        "答案应该提及 Cargo"
    );
}
...
async fn test_rag_qdrant_full_pipeline() {
    ...
    for (i, r) in results.iter().enumerate() {
        println!(
            "   {}. ID: {}",
            i + 1,
            r.document.id.as_ref().unwrap_or(&"N/A".to_string())
        );
...
async fn test_compare_memory_vs_qdrant() {
    ...
    // Qdrant 结果
    let qdrant_results = qdrant_retriever
        .retrieve_with_scores(query, 3)
        .await
        .unwrap();
    println!("\n📊 Qdrant 结果:");
    ...
    println!("\n✅ 验证: 两种数据库返回一致");
    assert_eq!(
        mem_results[0].document.content, qdrant_results[0].document.content,
        "两种数据库应返回相同的最相关文档"
    );
}
...
async fn test_rag_multi_turn_conversation() {
    ...
    // 验证答案准确性
    assert!(
        resp2.content.contains("Python")
            || resp2.content.contains("JavaScript")
            || resp2.content.contains("Rust"),
        "答案应该提及支持的编程语言"
    );
}

...
use langchainrust::{
    Document, InMemoryVectorStore, MockEmbeddings, MultiQueryConfig, RetrieverTrait,
    SimilarityRetriever, StaticQueryGenerator,
};
...
async fn test_static_query_generator_combined() {
    let synonyms: HashMap<String, Vec<String>> = HashMap::from([(
        "错误".to_string(),
        vec!["exception".to_string(), "异常".to_string()],
    )]);
...
async fn test_retriever_with_scores() {
    ...
    let docs = vec![Document::new("文档 A"), Document::new("文档 B")];
...
async fn test_static_generator_no_duplicates() {
    let generator = StaticQueryGenerator::new().with_prefix_expansion(vec!["test".to_string()]);
...
fn test_config_prompt_custom() {
    ...
    let config = MultiQueryConfig::new().with_prompt(custom_prompt.clone());

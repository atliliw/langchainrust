...
use langchainrust::{
    CallbackHandler, CallbackManager, FileCallbackHandler, LogFormat, Message, RunTree, RunType,
};
use std::sync::Arc;
...
mod tests {
    ...
    fn test_file_handler_format_configuration() {
        ...
        // 可以切换为其他格式
        let handler_json = FileCallbackHandler::new(temp_file.path())
            .unwrap()
            .with_format(LogFormat::Json);
        let handler_plain = FileCallbackHandler::new(temp_file.path())
            .unwrap()
    ...
    async fn test_all_callback_events_written_to_file() {
        ...
        handler
            .on_retriever_end(&run, &[serde_json::json!({})])
            .await;
    ...
    fn test_file_handler_integrates_with_callback_manager() {
        ...
        let manager = CallbackManager::new().add_handler(file_handler);

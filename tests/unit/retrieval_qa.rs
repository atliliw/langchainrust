...
use langchainrust::{BaseChain, Document, RetrievalQA, SimilarityRetriever};
...
async fn test_retrieval_qa_basic() {
    ...
    let inputs = HashMap::from([(
        "query".to_string(),
        Value::String("什么是 Rust？".to_string()),
    )]);
...
async fn test_retrieval_qa_with_sources() {
    ...
    let inputs = HashMap::from([(
        "query".to_string(),
        Value::String("什么是 LangChain？".to_string()),
    )]);
...
async fn test_retrieval_qa_custom_prompt() {
    ...
    retriever
        .add_documents(vec![Document::new("Go 语言由 Google 开发，注重并发编程。")])
        .await
        .unwrap();
...
简短回答：";
    ...
    let inputs = HashMap::from([(
        "query".to_string(),
        Value::String("Go 语言是谁开发的？".to_string()),
    )]);
...
async fn test_retrieval_qa_query_interface() {
    ...
    retriever
        .add_documents(vec![Document::new(
            "TypeScript 是 JavaScript 的类型超集，增加了静态类型检查。",
        )])
        .await
        .unwrap();

    let qa = RetrievalQA::new(config.openai_chat(), retriever).with_verbose(true);

    println!("\n=== 测试：query 简化接口 ===");

    let answer = qa
        .query("TypeScript 和 JavaScript 的关系是什么？")
        .await
        .unwrap();

...
use langchainrust::{BaseChain, LLMChain, LLMRouterChain, RouterChain};
...
async fn test_router_keywords_match() {
    ...
    let inputs1 = HashMap::from([(
        "input".to_string(),
        Value::String("帮我计算 1+1".to_string()),
    )]);
    ...
    let inputs2 = HashMap::from([(
        "input".to_string(),
        Value::String("如何写 Rust 代码".to_string()),
    )]);
    ...
    println!("\n--- 测试默认 Chain ---");
    let inputs3 = HashMap::from([("input".to_string(), Value::String("你好".to_string()))]);
...
async fn test_llm_router_intelligent() {
    ...
    let math_chain =
        Arc::new(LLMChain::new(config.openai_chat(), "计算: {input}").with_input_key("input"));
    let weather_chain =
        Arc::new(LLMChain::new(config.openai_chat(), "天气信息: {input}").with_input_key("input"));
    let general_chain =
        Arc::new(LLMChain::new(config.openai_chat(), "回答: {input}").with_input_key("input"));

    let router = LLMRouterChain::new(config.openai_chat())
        .add_route(
            "数学",
            "处理数学计算、加减乘除、数值运算相关问题",
            math_chain,
        )
        .add_route(
            "天气",
    ...
    let inputs = HashMap::from([(
        "input".to_string(),
        Value::String("今天北京的天气怎么样".to_string()),
    )]);
...
async fn test_router_fallback() {
    ...
    let router_no_default = RouterChain::new().add_route_with_keywords(
        "数学",
        "处理数学问题",
        math_chain.clone(),
        vec!["计算"],
    ...
    println!("\n--- 没有默认 Chain ---");
    let inputs1 = HashMap::from([("input".to_string(), Value::String("你好".to_string()))]);
    ...
    println!("\n--- 有默认 Chain ---");
    let inputs2 = HashMap::from([("input".to_string(), Value::String("你好".to_string()))]);
